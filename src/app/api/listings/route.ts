...
export async function POST(req: NextRequest) {
  ...
  if (!user) {
    return NextResponse.json({ error: "Nisi prijavljen (nema sesije na serveru)." }, { status: 401 });
  }

  const { data: sessionData } = await supabase.auth.getSession();
  const accessToken = sessionData.session?.access_token;
  if (!accessToken) {
    return NextResponse.json({ error: "Session postoji ali bez access tokena." }, { status: 401 });
  }
